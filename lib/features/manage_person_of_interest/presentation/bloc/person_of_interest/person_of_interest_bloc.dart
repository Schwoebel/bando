...
class PersonOfInterestBloc
  ...
  Stream<PersonOfInterestState> mapEventToState(
    ...
    if (event is ReadPersonOfInterestEvent) {} else
    if (event is ReadAllowedPersonsOfInterestEvent) {
      final result = await managePersonOfInterest.readList();
      yield* _failureOrPersonsOfInterest(result);
    } else if (event is CreatePersonOfInterestEvent) {} else
    if (event is DeletePersonOfInterestEvent) {} else
    if (event is UpdatePersonOfInterestEvent) {} else {
      yield InitialPersonOfInterestState();
    }
  }

  _failureOrPersonsOfInterest(
    Either<Failure, List<PersonOfInterest>> result) async* {
    yield result.fold(
        (Failure failure) =>
        ErrorWithPersonOfInterestProcess(
